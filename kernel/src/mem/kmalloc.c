...
static inline uint8_t* slab_stack_base(slab_hdr*h){return (uint8_t*)(h+1);}
static inline uint16_t* slab_stack_ptr(slab_hdr*h){return (uint16_t*)slab_stack_base(h);}
static inline uint8_t* slab_objs_base(slab_hdr*h){return slab_stack_base(h)+h->capacity*sizeof(uint16_t);}
static inline void* slab_obj_at(slab_hdr*h, uint16_t idx){return slab_objs_base(h)+((size_t)idx*h->obj_size);}

static inline uint64_t* arena_bitmap(arena_hdr* a){return (uint64_t*)(a+1);}
...
static slab_hdr* km_new_slab(uint32_t class_idx){
    void* page=kalloc(); if(!page) return NULL;
    uint32_t obj_size=km_class_idx_to_size(class_idx);
    uint8_t* p=(uint8_t*)page;
    slab_hdr* h=(slab_hdr*)p;
    *h=(slab_hdr){.next=NULL,.obj_size=(uint16_t)obj_size,.capacity=0,.free_count=0,.class_idx=(uint16_t)class_idx,.magic=KM_MAGIC_SMALL,.top=0};
    ...
    h->capacity=(uint16_t)cap; h->free_count=(uint16_t)cap; h->top=(uint16_t)cap;
...
static inline void* km_alloc_from_bin_locked(km_class_bin*bin,uint32_t class_idx){
    slab_hdr* h=bin->head;
    if(!h){ h=km_new_slab(class_idx); if(!h) return NULL; h->next=NULL; bin->head=h; }
    if(h->top==0){
        slab_hdr* n=h->next;
        if(!n){ n=km_new_slab(class_idx); if(!n) return NULL; n->next=NULL; bin->head=n; h=n; }
        else { bin->head=n; h=n; }
    }
    uint16_t* stk=slab_stack_ptr(h);
    uint16_t idx=stk[--h->top];
    h->free_count--;
    if(h->top==0){ slab_hdr* n=h->next; bin->head=(n?n:h); if(n) h->next=NULL; }
...
static inline void km_free_to_bin_locked(km_class_bin*bin,slab_hdr*h,void*ptr){
    ...
    stk[h->top++]=idx;
    h->free_count++;
    if(h->free_count==1){ h->next=bin->head; bin->head=h; }
    if(h->free_count==h->capacity){ if(bin->head==h) bin->head=h->next; kfree((void*)h); }
}
